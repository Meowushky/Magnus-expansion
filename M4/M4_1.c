...
typedef struct
{
  ...
  uint64_t calls;
  double step; //величина шага
} rwf_ctx;

double ex(double);
void EigenV(double*, double, double);
void aWF_calc(wf_ctx*, rwf_ctx*, double complex*);

void aWF_calc(wf_ctx *ctx, rwf_ctx *res, double complex *APsi)
{
  _Bool flag=0;
...
{  
  double a=4351960.,b=0.030554,E=1.0,tol=0.00001;
  ...
  aWF_calc(&ctx,&res,APsi_0);
  
  for(int j1=0;j1<FLAVS;j1++)
    Psi_p[j1]=(res.Psi[j1]-psi_0[j1])/delta;
  ...
  //правая часть равенства -- компоненты -IHPsi
  fprintf(stderr,"%lf+I%lf; ",creal(APsi_0[0]),cimag(APsi_0[0]));
  fprintf(stderr,"%lf+I%lf; ",creal(APsi_0[1]),cimag(APsi_0[1]));
  fprintf(stderr,"%lf+I%lf; \n",creal(APsi_0[2]),cimag(APsi_0[2]));
  
  printf("%lf\t%lf\t%ld\t%lf\n",d1,Pee,res.calls,delta);
